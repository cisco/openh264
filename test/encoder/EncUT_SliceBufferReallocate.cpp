...
TEST_F(CSliceBufferReallocatTest, ReallocateTest_01) {
    ...
    ASSERT_TRUE(cmResultSuccess == iRet);
    ASSERT_TRUE(NULL != pCtx->pCurDqLayer->sSliceBufferInfo[iThreadIndex].pSliceBuffer);
    ASSERT_TRUE(iSlcBufferNum < pCtx->pCurDqLayer->sSliceBufferInfo[iThreadIndex].iMaxSliceNum);
